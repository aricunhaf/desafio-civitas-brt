-- back compat for old kwarg name
    ...
    merge into `desafio-civitas-brt-ari`.`brt_data`.`dbt_invocations` as DBT_INTERNAL_DEST
        using (
        select
        * from `desafio-civitas-brt-ari`.`brt_data`.`dbt_invocations__dbt_tmp`
        ) as DBT_INTERNAL_SOURCE
        on ((DBT_INTERNAL_SOURCE.invocation_id = DBT_INTERNAL_DEST.invocation_id))

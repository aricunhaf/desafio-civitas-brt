-- back compat for old kwarg name
    ...
    merge into `desafio-civitas-brt-ari`.`brt_data`.`elementary_test_results` as DBT_INTERNAL_DEST
        using (
        select
        * from `desafio-civitas-brt-ari`.`brt_data`.`elementary_test_results__dbt_tmp`

  ...
  create or replace view `desafio-civitas-brt-ari`.`brt_data`.`snapshot_run_results`
  OPTIONS()
  as 

with dbt_run_results as (
    select * from `desafio-civitas-brt-ari`.`brt_data`.`dbt_run_results`
),

dbt_snapshots as (
    select * from `desafio-civitas-brt-ari`.`brt_data`.`dbt_snapshots`
)

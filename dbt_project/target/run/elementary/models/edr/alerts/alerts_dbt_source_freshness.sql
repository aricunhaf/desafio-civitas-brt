  ...
  create or replace view `desafio-civitas-brt-ari`.`brt_data`.`alerts_dbt_source_freshness`
  OPTIONS()
  as 

with results as (
  select * from `desafio-civitas-brt-ari`.`brt_data`.`dbt_source_freshness_results`
),

sources as (
  select * from `desafio-civitas-brt-ari`.`brt_data`.`dbt_sources`
)
